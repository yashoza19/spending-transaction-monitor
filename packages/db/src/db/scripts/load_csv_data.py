...
async def clear_existing_data(session) -> None:
    ...
    try:
        ...
        try:
            ...
            await session.execute(text("SELECT setval(pg_get_serial_sequence('transactions', 'id'), 1, false);"))
            await session.execute(text("SELECT setval(pg_get_serial_sequence('alert_rules', 'id'), 1, false);"))
            await session.execute(text("SELECT setval(pg_get_serial_sequence('credit_cards', 'id'), 1, false);"))
            print("   Reset auto-increment sequences")
        except Exception as seq_error:
            # Sequences may not exist or may have different names, which is okay
            print(f"   Note: Could not reset sequences (this is usually okay): {seq_error}")
    ...
    try:
        ...
        for row, original_created_at in users_data:
            ...
            user = User(
                ...
                credit_limit=Decimal(row.get('credit_limit', '0')) if row.get('credit_limit') else None,
                credit_balance=Decimal(row.get('credit_balance', '0')) if row.get('credit_balance') else None,
        ...
        await session.commit()
        print(f"✅ Successfully loaded {users_added} users from CSV with adjusted timestamps")
...
async def load_transactions_from_csv(session, csv_path: str, user_id_mapping: Dict[str, str]) -> None:
    ...
    try:
        ...
        with open(csv_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                if not user_id:
                    print(f"Warning: User ID {row['user_id']} not found in user mapping, skipping transaction")
                    continue
        ...
        if latest_transaction_date:
            ...
            time_offset = None
            print("⚠️  No transaction dates found, using current time for all transactions")
        ...
        for row, user_id, original_date in transactions_data:
            ...
            transaction = Transaction(
                ...
                merchant_latitude=float(row['merchant_latitude']) if row.get('merchant_latitude') else None,
                merchant_longitude=float(row['merchant_longitude']) if row.get('merchant_longitude') else None,
        ...
        await session.commit()
        print(f"✅ Successfully loaded {transactions_added} transactions from CSV with adjusted timestamps")
        ...
        if transactions_data and time_offset:
            ...
            print(f"   • All {len(transactions_data)} transactions shifted by the same amount")
...
async def load_csv_data(users_csv_path: str = '/app/data/sample_users.csv', 
                       transactions_csv_path: str = '/app/data/sample_transactions.csv') -> None:
    ...
    async with SessionLocal() as session:
        try:
            ...
            await load_transactions_from_csv(session, transactions_csv_path, user_id_mapping)
