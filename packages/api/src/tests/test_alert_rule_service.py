...
from db.models import AlertRule, AlertNotification, Transaction, AlertType
...
class TestAlertRuleService:
    ...
    @pytest.mark.asyncio
    async def test_validate_alert_rule_success_with_real_transaction(self, alert_rule_service, mock_session, sample_transaction):
        ...
        with patch.object(alert_rule_service.transaction_service, 'get_latest_transaction', return_value=sample_transaction):
            ...
            with patch.object(AlertRuleService, 'parse_nl_rule_with_llm') as mock_parse:
                ...
                # Act
                result = await alert_rule_service.validate_alert_rule(rule_text, user_id, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_validate_alert_rule_success_with_dummy_transaction(self, alert_rule_service, mock_session):
        ...
        with patch.object(alert_rule_service.transaction_service, 'get_latest_transaction', return_value=None):
            # Mock the dummy transaction
            dummy_transaction = {'user_id': user_id, 'amount': 100.0}
            with patch.object(alert_rule_service.transaction_service, 'get_dummy_transaction', return_value=dummy_transaction):
                # Mock the LLM parsing to return valid result
                with patch.object(AlertRuleService, 'parse_nl_rule_with_llm') as mock_parse:
                    mock_parse.return_value = {
                        'valid_sql': True,
    ...
    @pytest.mark.asyncio
    async def test_validate_alert_rule_invalid_sql(self, alert_rule_service, mock_session, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_validate_alert_rule_llm_error(self, alert_rule_service, mock_session, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_trigger_alert_rule_success(self, alert_rule_service, mock_session, sample_alert_rule, sample_transaction):
        """Test successful triggering of an alert rule"""
        # Arrange
        # Mock the transaction service to return a transaction
        with patch.object(alert_rule_service.transaction_service, 'get_latest_transaction', return_value=sample_transaction):
            # Mock the LLM generation to return trigger result
            with patch.object(AlertRuleService, 'generate_alert_with_llm') as mock_generate:
                ...
                # Act
                result = await alert_rule_service.trigger_alert_rule(sample_alert_rule, mock_session)
    ...
    @pytest.mark.asyncio
    async def test_trigger_alert_rule_not_triggered(self, alert_rule_service, mock_session, sample_alert_rule, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_trigger_alert_rule_inactive_rule(self, alert_rule_service, mock_session, inactive_alert_rule):
        """Test triggering an inactive alert rule"""
        # Act & Assert
        with pytest.raises(ValueError, match='Alert rule is not active'):
            await alert_rule_service.trigger_alert_rule(inactive_alert_rule, mock_session)

    @pytest.mark.asyncio
    async def test_trigger_alert_rule_no_transaction(self, alert_rule_service, mock_session, sample_alert_rule):
        ...
        with patch.object(alert_rule_service.transaction_service, 'get_latest_transaction', return_value=None):
            ...
            with pytest.raises(ValueError, match='No transaction found for user'):
                await alert_rule_service.trigger_alert_rule(sample_alert_rule, mock_session)

    @pytest.mark.asyncio
    async def test_trigger_alert_rule_llm_error(self, alert_rule_service, mock_session, sample_alert_rule, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_validate_alert_rule_none_result_from_llm(self, alert_rule_service, mock_session, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_trigger_alert_rule_none_result_from_llm(self, alert_rule_service, mock_session, sample_alert_rule, sample_transaction):
    ...
    @pytest.mark.asyncio
    async def test_trigger_alert_rule_creates_correct_notification(self, alert_rule_service, mock_session, sample_alert_rule, sample_transaction):
        ...
        with patch.object(alert_rule_service.transaction_service, 'get_latest_transaction', return_value=sample_transaction):
            ...
            with patch.object(AlertRuleService, 'generate_alert_with_llm') as mock_generate:
                ...
                call_args = mock_session.add.call_args[0][0]  # Get the notification object
