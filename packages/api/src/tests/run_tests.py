...
def run_command(cmd, capture_output=False):
    ...
    result = subprocess.run(
        cmd, capture_output=capture_output, text=True, cwd=Path(__file__).parent
    )
...
Examples:
  ...
  python run_tests.py --verbose         # Run tests with verbose output
    ...
    test_group.add_argument(
        '--transaction', action='store_true', help='Run transaction service tests only'
    )
    test_group.add_argument(
        '--alert-rule', action='store_true', help='Run alert rule service tests only'
    )
    ...
    # Run the tests
    success = run_tests(
        test_file=test_file, coverage=args.coverage, verbose=args.verbose
    )
