...
custom_rule = st.text_input(
    ...
    value=st.session_state.get(
        'selected_rule', 'Alert me when I spend more than $100 on dining'
    ),
...
st.markdown(
    "Test your rule structure and logic with the user's latest transaction from the database"
)

# Database status check
if DATABASE_AVAILABLE:
    db_status = (
        test_database_connection() if 'test_database_connection' in globals() else False
    )
    if db_status:
...
with col1:
    if st.button('🔍 Validate Rule', type='secondary', use_container_width=True):
        if not custom_rule.strip():
            ...
            st.error(
                'Database connection not available. Using default transaction data.'
            )
            ...
            try:
                ...
                st.info(
                    f'📄 Using transaction: {latest_transaction.get("trans_num")} - ${latest_transaction.get("amount")} at {latest_transaction.get("merchant_name")}'
                )
                ...
                with st.spinner('Validating rule...'):
                    res = requests.post(
                        ...
                        json={
                            'natural_language_query': custom_rule,
                            'user_id': user_id,
                        },
                    )
...
with col2:
    if 'result' in locals() and result.get('status') == 'valid':
        st.info(
            '✅ Rule validation successful! You can now test it with transaction data below.'
        )
    else:
...
st.markdown(
    'Test your rule against actual transaction data to see if alerts are triggered'
)
    ...
    if st.button('🚨 Generate Alert', type='primary', use_container_width=True):
        if not custom_rule.strip():
            ...
            try:
                ...
                st.error(
                    '❌ Invalid JSON in transaction data. Please check the format.'
                )
            except requests.exceptions.ConnectionError:
                st.error(
                    "🔌 Cannot connect to API server. Make sure it's running on localhost:8000"
...
with col4:
    st.info(
        "💡 Click 'Generate Alert' to test your rule against the transaction data. Modify the transaction amount, merchant, or category to test different scenarios."
    )
...
try:
    ...
    st.error(
        '❌ API server is not responding. Start it with: `uvicorn main:app --reload`'
    )
