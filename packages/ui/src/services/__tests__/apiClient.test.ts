...
describe('ApiClient', () => {
  ...
  describe('JWT Token Retrieval', () => {
    it('should find JWT token with exact OIDC key pattern', async () => {
      ...
      const oidcKey = 'oidc.user:http://localhost:8080/realms/spending-monitor:spending-monitor';
      ...
      expect(consoleLog).toHaveBeenCalledWith('✅ FOUND JWT TOKEN');
    });
    ...
    it('should find JWT token with fallback key scanning', async () => {
      ...
      const fallbackKey = 'oidc.user:https://keycloak.example.com/realms/test:test-client';
      ...
      expect(consoleLog).toHaveBeenCalledWith(`✅ FOUND JWT TOKEN in fallback: ${fallbackKey}`);
    });

    it('should make unauthenticated request when no JWT token found', async () => {
      // Arrange: No tokens in localStorage
      mockFetch.mockResolvedValueOnce(new Response('{"detail":"Authentication required"}', { status: 401 }));
      ...
      expect(consoleWarn).toHaveBeenCalledWith('❌ NO JWT TOKEN FOUND');
      expect(consoleWarn).toHaveBeenCalledWith('⚠️  DEBUG: Making unauthenticated request to: /api/test');
    });
    ...
    it('should handle corrupted localStorage gracefully', async () => {
      ...
      localStorage.setItem('oidc.user:fallback:client', '{"access_token": "backup-token"}');
    ...
    it('should handle missing access_token in valid OIDC object', async () => {
      ...
      // Assert - Should make unauthenticated request
      expect(consoleWarn).toHaveBeenCalledWith('❌ NO JWT TOKEN FOUND');
      expect(mockFetch).toHaveBeenCalledWith('/api/test', {
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('HTTP Request Handling', () => {
    it('should preserve existing headers while adding authentication', async () => {
      ...
      localStorage.setItem('oidc.user:test:client', JSON.stringify({
        access_token: mockToken
      }));
  ...
  describe('Real-world OIDC Token Patterns', () => {
    it('should handle Keycloak production token structure', async () => {
      ...
      const keycloakToken = {
        access_token: 'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJrZXljbG9hay1pZCJ9..signature',
        token_type: 'Bearer',
        expires_in: 300,
    ...
    it('should work with Auth0 token structure', async () => {
      ...
      localStorage.setItem('oidc.user:https://dev-auth0.auth0.com:auth0-client', JSON.stringify(auth0Token));
  ...
  describe('Error Scenarios', () => {
    ...
    it('should handle extremely long localStorage keys', async () => {
      ...
      localStorage.setItem(longKey, JSON.stringify({
